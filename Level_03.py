...
def main():
    ...
    tmx_data = load_pygame("my_map_3.tmx")
    ...
    final_convert = pygame.image.load("imges/Final convert (1).png").convert_alpha()
    final_Level = pygame.image.load("imges/Final level (1).png").convert_alpha()
    ...
    x = 0
    y = y_ground
    world_offset = [-100,-2600]  # start:- (-100, -2600) (-800, -1600)
    R_G = 1
    Key_time = 19 * 4
    Key_time_0 = 19 * 4
    ...
    while not quit:
        ...
        standing_on_left = get_my_tiles_properties(tmx_data, x + 10, y + 70, world_offset)
        standing_on_right = get_my_tiles_properties(tmx_data, x + 50 - 10, y + 70, world_offset)
        ...
        if touching_locks_keys_rightside["require"] in collected_items:
            ...
            if item == "green key":
                tmx_data.layers[0].data[touching_locks_keys_rightside["y"]][touching_locks_keys_rightside["x"]] = 0
                tmx_data.layers[0].data[57][89] = tmx_data.layers[1].data[57][89]
                tmx_data.layers[0].data[58][89] = tmx_data.layers[1].data[58][89]
                next_level.play()



        if standing_on_right["x"] == 92 and standing_on_right["y"] == 59:
            can_not_move = 0
            C_H.points = points
            C_H.health = health
            Key_time -= 1
            window.blit(final_convert, (0,0))
            if Key_time <= 0:
                Key_time_0 -= 1
                window.blit(final_Level, (0,0))
            if Key_time_0 <= 0:
                C_H.health = C_H.health + C_H.points//20
                quit = True
