...
def main():
    ...
    tmx_data = load_pygame("my_map_0.tmx")
    ...
    while not quit:
        ...
        if touching_locks_keys_rightside["require"] in collected_items:
            ...
            if item == "green key":
                tmx_data.layers[0].data[touching_locks_keys_rightside["y"]][touching_locks_keys_rightside["x"]] = 0
                tmx_data.layers[0].data[21][94] = tmx_data.layers[1].data[21][94]
                tmx_data.layers[0].data[22][94] = tmx_data.layers[1].data[22][94]
                next_level.play()




        if touching_coins_right["x"] == 98:
            C_H.points = points
            C_H.health = health
            quit = True
