...
def main():
    ...
    tmx_data = load_pygame("my_map_1.tmx")
    ...
    Alien_friend = pygame.image.load("imges/Alien friend (1).png")
    ...
    while not quit:
        ...
        if touching_locks_keys_leftside["require"] in collected_items:
            ...
            if item == "green key":
                tmx_data.layers[0].data[touching_locks_keys_leftside["y"]][touching_locks_keys_leftside["x"]] = 0
                tmx_data.layers[0].data[50][23] = tmx_data.layers[1].data[50][23]
                tmx_data.layers[0].data[51][23] = tmx_data.layers[1].data[51][23]
                next_level.play()



        if standing_on_left["x"] == 19 and standing_on_left["y"] == 52:
            can_not_move = 0
            C_H.points = points
            C_H.health = health
            window.blit(Alien_friend, (0,0))
            Key_time -= 1
            if Key_time <= 0:
                quit = True
